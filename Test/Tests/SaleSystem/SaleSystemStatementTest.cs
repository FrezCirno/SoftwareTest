...
namespace ProgramTest.Tests
{
	...
	public class SaleSystemStatementTest : Test
	{
		//测试文件的json地址
		private static string TEST_FILE = "../../mydata/SaleSystem/SaleSystem_Statement_Testcase.json";
		private static string TEST_RESULT = "../../mydata/SaleSystem/SaleSystem_Statement_Result.json";

		private Dictionary<string, SaleSystem1> ReadJsonFile()
		{
			using (StreamReader r = new StreamReader(TEST_FILE))
			{
				string json = r.ReadToEnd();
				Dictionary<string, SaleSystem1> saleDic = JsonConvert.DeserializeObject<Dictionary<string, SaleSystem1>>(json);
				return saleDic;
			}
		}
		...
		{
			Dictionary<string, SaleSystem1> saleDictionary = ReadJsonFile();
			Dictionary<string, double> resultDictionary = new Dictionary<string, double>();
			foreach (KeyValuePair<string, SaleSystem1> kvp in saleDictionary)
			{
